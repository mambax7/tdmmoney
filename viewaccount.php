...
$xoopsOption['template_main'] = 'tdmmoney_viewaccount.html';
...
if (!$gperm_handler->checkRight('tdmmoney_view', $account_id, $groups, $xoopsModule->getVar('mid'))) {
...
if (isset($_REQUEST['date_start']) && isset($_REQUEST['date_end'])){
    $date_start = strtotime(TDMMoney_CleanVars($_REQUEST, 'date_start', 0, 'int'));
    $date_end = strtotime(TDMMoney_CleanVars($_REQUEST, 'date_end', 0, 'int'));
}else{
    //sans filtre
...
if (count($operation_arr)>0) {
    ...
    $balance = $balance + $balance_ammount;
    ...
    foreach (array_keys($operation_arr) as $i) {
        ...
        $perm_modif = ($gperm_handler->checkRight('tdmmoney_ac', 8, $groups, $xoopsModule->getVar('mid'))) ? true : false ;
        ...
        if ($perm_modif == true){
            $action = '<a href="submit.php?op=edit&operation_id=' . $i . '"><img src="images/deco/edit.gif" alt="' . _MD_TDMMONEY_EDIT . '" title="' . _MD_TDMMONEY_EDIT . '"></a> ' .
                      '<a href="submit.php?op=del&operation_id=' . $i . '&account_id=' . $account_id . '"><img src="images/deco/delete.gif" alt="' . _MD_TDMMONEY_DEL . '" title="' . _MD_TDMMONEY_DEL . '"></a>';
        }
        $xoopsTpl->append('operation', array('operation_date' => formatTimeStamp($operation_arr[$i]->getVar('operation_date'), 's'), 'operation_sender' => $sender,
                          'operation_category' => $category, 'operation_description' => $operation_arr[$i]->getVar('operation_description'), 'operation_withdraw' => $withdraw,
                          'operation_deposit' => $deposit, 'operation_balance' => $display_operation_balance . ' ' . $operation_arr[$i]->getVar('account_currency'), 'operation_action' => $action));
    }
    //Solde initial
    ...
    $perm_pdf = ($gperm_handler->checkRight('tdmmoney_ac', 16, $groups, $xoopsModule->getVar('mid'))) ? true : false ;
    $xoopsTpl->assign('perm_pdf', $perm_pdf);

    //ajout selon les permissions
    $perm_add = ($gperm_handler->checkRight('tdmmoney_ac', 4, $groups, $xoopsModule->getVar('mid'))) ? true : false ;
