...
<div align="right">
    <form name="nav" action="journal.php" method="post">
        ...
        <{/if}>
        <{if $perm_modif != ""}>
        <{*<input type="button" value="<{$smarty.const._MD_TDMMONEY_VIEWACCOUNT_EXPORTPDF}>" onclick="location='include/operation_pdf.php?account_id=<{$accountId}>&date_start=<{$date_start}>&date_end=<{$date_end}>'">*}>
        <{if $displayPdf == 1}>
            <input type="button" value="<{$smarty.const._MD_TDMMONEY_VIEWACCOUNT_EXPORTPDF}>" onclick="location='makepdf.php?account_id=<{$accountId}>&date_start=<{$date_start}>&date_end=<{$date_end}>'">
        <{/if}>
        <{/if}>
    </form>
</div>
